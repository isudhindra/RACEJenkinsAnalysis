...
# ============================================================================
# QA SIMULATION TOGGLES
# ============================================================================
# All toggles are off by default. Trigger via query param or X-Mock-Simulate header.
#
# Fault-injection (?simulate=<mode>):
#   401          -- every /api/* returns 401 (verifies the new "Session expired" banner)
#   429          -- every /api/* returns 429 (verifies the rate-limit row in troubleshooting)
#   slow         -- every /api/* sleeps 4s (verifies spinner / Refreshing UX)
#   stream_drop  -- SSE stream drops mid-flight after 2-3 events
#   stage2_error -- one Stage-2 job emits an error event then stream continues
#   supersede    -- fetch_stream mutates active_operation_id mid-flight (cancellation test)
#
# View-size (?view_size=<size>):
#   empty        -- 0 jobs (empty view edge case)
#   single       -- 1 job
#   large        -- 220 jobs (SSE keep-alive / KPI counter stress test)
#
# Example: open http://localhost:5111/?simulate=401 or curl ...?view_size=large
# ============================================================================
...
# ===== QA SIMULATION TOGGLES =====
# ============================================================================
# Fault-injection + view-size scenario toggles for visual QA only.
#
# Concurrency note: Flask's dev server with threaded=True can serve concurrent
# requests, but in practice QA toggles are exercised one-at-a-time from a
# single browser tab. Module-level booleans are therefore acceptable; we reset
# them in @app.after_request so they don't leak between requests.

_SIMULATE_STREAM_DROP = False
_SIMULATE_STAGE2_ERROR = False
_SIMULATE_SUPERSEDE = False


def _simulate_mode():
    """Resolve the current ?simulate=... mode (query string OR header).
    Returns lowercase string or empty string when no toggle is active."""
    mode = (request.args.get("simulate") or "").strip().lower()
    if not mode:
        mode = (request.headers.get("X-Mock-Simulate") or "").strip().lower()
    return mode


def _view_size():
    """Resolve the current ?view_size=... scenario (query string OR header)."""
    size = (request.args.get("view_size") or "").strip().lower()
    if not size:
        size = (request.headers.get("X-Mock-View-Size") or "").strip().lower()
    return size


@app.before_request
def _qa_simulate_before():
    """Inspect ?simulate=... and short-circuit /api/* responses where requested.

    Notes:
      - /api/config is always allowed through so the dashboard can boot.
      - SSE-only modes (stream_drop / stage2_error / supersede) set module-level
        flags that the relevant generators check, rather than returning early.
    """
    global _SIMULATE_STREAM_DROP, _SIMULATE_STAGE2_ERROR, _SIMULATE_SUPERSEDE
    ...
    mode = _simulate_mode()
    if not mode or mode == "ok":
        return None

    if mode == "401":
        return jsonify({"error": "Mock auth gate — simulate=401"}), 401

    if mode == "429":
        return jsonify({"error": "Mock rate-limit — simulate=429", "retry_after": 5}), 429

    if mode == "slow":
        # Just delay — let the real handler run after the sleep.
        time.sleep(4)
        return None

    if mode == "stream_drop":
        _SIMULATE_STREAM_DROP = True
        return None

    if mode == "stage2_error":
        _SIMULATE_STAGE2_ERROR = True
        return None

    if mode == "supersede":
        _SIMULATE_SUPERSEDE = True
        return None

    # Unknown mode — pass through, treat as no-op.
    return None


@app.after_request
def _qa_simulate_after(response):
    """Reset SSE-influencing flags so toggles don't leak across requests."""
    global _SIMULATE_STREAM_DROP, _SIMULATE_STAGE2_ERROR, _SIMULATE_SUPERSEDE
    _SIMULATE_STREAM_DROP = False
    _SIMULATE_STAGE2_ERROR = False
    _SIMULATE_SUPERSEDE = False
...
def _synth_view_size_jobs(size):
    """Build a list of synthesised jobs for ?view_size=... scenarios.

    Returns None when the size token doesn't request synthesis (so the caller
    falls back to the normal generate_mock_jobs() behaviour).
    """
    if size == "empty":
        return []
    if size == "single":
        return [{
            "job_name": "Mock single job",
            "job_url": f"{BASE}/job/mock-single-job/",
            ...
            "error_message": None,
            "three_run_context": {
                "latest": {"build_number": 1, "status": "SUCCESS",
    ...
    if size == "large":
        ...
        for i in range(220):
            # Sprinkle a small handful of FAILED jobs so Stage-2 still streams
            # something interesting; the bulk stay SUCCESS for cheap throughput.
            is_failed = (i % 20 == 0)
            status = "FAILURE" if is_failed else "SUCCESS"
            health = "FAILED" if is_failed else "PASSED"
            jobs.append({
                "job_name": f"large-view-job-{i}",
                "job_url": f"{BASE}/job/large-view-job-{i}/",
                "current_status": status,
                "health_state": health,
                "is_running": False,
                "last_execution_time": now_iso,
                "last_build_number": 1,
                ...
                "error_message": None,
                "three_run_context": {
                    "latest": {"build_number": 1, "status": status,
                               "timestamp": now_iso, "duration_ms": 60000},
                    "previous": None,
                    "last_passed": None,
                },
                "test_metrics": {"metrics_unavailable": True, "metrics_source": None},
                "classification": ({
                    ...
                    "action": "n/a",
                    "label": "Stress Test",
                    "all_labels": [{"label": "Stress Test", "domain": "Unknown",
                                    "action": "n/a", "rule_name": ""}],
                    "secondary_hint": None,
        ...
        return jobs
    # "normal" or empty token → fall through to default
    return None
...
def dashboard():
    ...
    # Pass dicts (not JSON strings) — the template uses `|tojson` which
    # expects native Python objects. Passing a pre-stringified value
    # double-encodes it and breaks the JSON.parse() on the browser side
    # (contexts.instances becomes undefined → instance dropdown stays empty).
    return render_template(
        "dashboard.html",
        contexts=MOCK_CONTEXTS,
        analysis_taxonomy=taxonomy,
        auto_refresh_interval_ms=_mock_auto_refresh_interval_ms(),
        # Mock has no token guard, but the template injects this into a meta
        # tag that apiFetch() reads — keep it a non-empty placeholder so the
        # X-RACE-Token header is always present.
        local_api_token="mock-token",
    )
...
def discover_view_jobs_count():
    ...
    # QA view-size override
    size = _view_size()
    if size == "empty":
        return jsonify({"count": 0, "view_name": "Mock empty view"}), 200
    if size == "single":
        return jsonify({"count": 1, "view_name": "Mock single-job view"}), 200
    if size == "large":
        return jsonify({"count": 220, "view_name": "Mock large view (220 jobs)"}), 200
...
def fetch_stream():
    ...
    # QA view-size override — synthesise rather than use the default fixture
    size = _view_size()
    synth = _synth_view_size_jobs(size)
    ...
    # Capture SSE-only simulate flags into the closure — @app.after_request will
    # reset the module-level flags before the generator actually runs.
    do_stream_drop = _SIMULATE_STREAM_DROP
    do_stage2_error = _SIMULATE_STAGE2_ERROR
    do_supersede = _SIMULATE_SUPERSEDE

    data = request.get_json() or {}
    view_path = data.get("view_path", "")

    # Filter based on selected view (skip filter for synthesised QA scenarios)
    if synth is not None:
        target_jobs = all_jobs
    elif "Failed" in view_path:
    ...
    # Empty view_size short-circuit — emit only fetch_complete.
    if size == "empty":
        def generate_empty():
            yield _format_sse({
    ...
    def generate():
        ...
        for idx, job in enumerate(target_jobs):
            ...
            # QA: stream_drop — bail after 2-3 events to simulate network drop.
            if do_stream_drop and events_emitted >= 2:
                raise GeneratorExit

            # QA: supersede — rotate active_operation_id after 3-4 events so
            # the next loop sees the change and bails (mirrors a second fetch
            # arriving while the first is still streaming).
            if do_supersede and events_emitted >= 3:
        ...
        for idx, job in enumerate(failed_jobs):
            ...
            # QA: stage2_error — after 2 normal job events, inject a single
            # error event then keep streaming the rest normally.
            if do_stage2_error and not stage2_error_emitted and idx >= 1:
                yield _format_sse({
                    "event_type": "error",
                    "operation_id": operation_id,
                    "job_url": job["job_url"],
                    "error": "Mock Stage-2 exception",
                })
                stage2_error_emitted = True
...
def refresh_stream():
    ...
    # Capture stream_drop flag — after_request resets it before generator runs.
    do_stream_drop = _SIMULATE_STREAM_DROP
    ...
    def generate():
        ...
        for idx, job in enumerate(targets):
            ...
            # QA: stream_drop — simulate network drop mid-flight.
            if do_stream_drop and events_emitted >= 2:
                raise GeneratorExit
...
# ============================================================================
# AUTO-REFRESH POLL (#276) — view-batched + per-job fallback simulation
# ============================================================================

# Tick counter drives a deterministic change simulator so opening the
# dashboard for ~1 minute shows real status flips during auto-refresh
# (rather than every job staying SUCCESS forever).
_POLL_TICK_LOCK = threading.Lock()
_poll_tick = {"n": 0}


def _normalise_url(url):
    """Mirror the backend helper - strip trailing slash for stable comparison."""
    return (url or "").rstrip("/")


def _ts_iso():
    """ISO timestamp matching the real backend per-poll format."""
    return time.strftime("%Y-%m-%dT%H:%M:%S")


def _drifted_status_for(job, tick):
    """Deterministically flip a handful of jobs over time so auto-refresh
    actually demonstrates change detection. Roughly 1 job in 30 swaps
    SUCCESS<->FAILURE every 4 ticks (~2 min at the default 30 s
    cadence). All other jobs stay rock-stable so the demo isolates the
    flipped row clearly. IN_PROGRESS jobs are left alone - they're
    already exercising the running-state path."""
    base_status = job.get("current_status", "SUCCESS")
    base_running = bool(job.get("is_running", False))
    base_build = (job.get("three_run_context", {}).get("latest", {}) or {}).get("build_number", 1)
    seed = hash(job.get("job_url", "")) & 0xFFFF
    bucket = seed % 30
    if bucket > 1 or base_running:
        return base_build, base_status, base_running
    phase = (tick + (seed // 30)) % 4
    if phase < 2:
        return base_build, base_status, base_running
    flipped = "FAILURE" if base_status == "SUCCESS" else "SUCCESS"
    return base_build + 1, flipped, False


@app.route("/api/poll-status", methods=["POST"])
def poll_status():
    """Mirror the real poll-status: batched-view path or per-job fallback.

    Two paths:
      - ``source_mode == "view"`` + ``view_url``: returns ALL jobs in one
        batched response with ``diag.jenkins_calls == 1`` so the frontend
        can verify the optimisation is firing.
      - Otherwise (custom job list, no view_url, or any URL missing from
        the batched view): per-job fallback with
        ``diag.jenkins_calls == len(remaining)``.

    Each entry includes ``is_running`` so the frontend's 3-field
    signature works. Status drifts deterministically over ticks so
    leaving the dashboard open demonstrates the diff machinery in action.
    """
    body = request.get_json() or {}
    job_urls = body.get("job_urls", []) or []
    source_mode = (body.get("source_mode") or "").strip()
    view_url = (body.get("view_url") or "").strip()

    with _POLL_TICK_LOCK:
        _poll_tick["n"] += 1
        tick = _poll_tick["n"]

    by_url = {_normalise_url(j["job_url"]): j for j in mock_jobs}

    statuses = []
    covered = set()
    batched_used = source_mode == "view" and bool(view_url)

    if batched_used:
        for url in job_urls:
            job = by_url.get(_normalise_url(url))
            if job is None:
                continue
            build_num, status, is_running = _drifted_status_for(job, tick)
            statuses.append({
                "job_url": url,
    ...
    for url in remaining:
        ...
        if job is None:
            statuses.append({
                ...
                "timestamp": None,
                "is_running": False,
                "error": "Mock: job not found in the simulated view/job-list",
            })
            continue
        build_num, status, is_running = _drifted_status_for(job, tick)
        statuses.append({
    ...
    return jsonify({"statuses": statuses, "diag": diag}), 200


# ============================================================================
# CLASSIFY-TEST SANDBOX (#271) - paste-a-log endpoint for rule authors
# ============================================================================

# Cache the real RACE Classifier so we load YAML rules once per process,
# not on every /api/classify-test call. The mock binds to a single thread
# in the dev server so plain module-level state is fine.
_REAL_CLASSIFIER = None
_REAL_CLASSIFIER_TRIED = False
_REAL_CLASSIFIER_ERROR = None


def _get_real_classifier():
    """Load the real RACE Classifier from ../config/rules. Cached after first call.
    Returns None and prints a one-time warning if it can't be loaded.
    """
    global _REAL_CLASSIFIER, _REAL_CLASSIFIER_TRIED, _REAL_CLASSIFIER_ERROR
    if _REAL_CLASSIFIER_TRIED:
        return _REAL_CLASSIFIER
    _REAL_CLASSIFIER_TRIED = True
    try:
        ...
        if not os.path.isdir(rules_dir):
            _REAL_CLASSIFIER_ERROR = f"rules directory not found at {rules_dir}"
            print(
                f"[mock] WARN: /api/classify-test falling back to keyword "
                f"heuristic — {_REAL_CLASSIFIER_ERROR}"
            )
            return None
        _REAL_CLASSIFIER = Classifier(rules_path=rules_dir)
        print(
            f"[mock] /api/classify-test using REAL classifier — "
            f"{len(_REAL_CLASSIFIER.rules)} rules loaded from {rules_dir}"
        )
        return _REAL_CLASSIFIER
    except Exception as exc:
        _REAL_CLASSIFIER_ERROR = repr(exc)
        print(
            f"[mock] WARN: /api/classify-test falling back to keyword "
            f"heuristic — could not load real classifier: {exc}"
        )
        return None


def _build_sandbox_classifier(candidate_yaml):
    """Build a transient Classifier with live rules + the candidate YAML overlaid.
    Mirrors the real handler in race/routes/classify_test.py.
    """
    ...
    parsed = _yaml.safe_load(candidate_yaml)
    if not isinstance(parsed, dict) or "rules" not in parsed:
        raise ValueError("Candidate YAML must be a mapping with a top-level 'rules' list")

    project_root = os.path.dirname(os.path.dirname(os.path.abspath(__file__)))
    live_rules_dir = os.path.join(project_root, "config", "rules")

    with tempfile.TemporaryDirectory(prefix="mock-sandbox-") as tmp:
        if os.path.isdir(live_rules_dir):
        ...
        with open(os.path.join(tmp, "zz-sandbox-candidate.yaml"), "w") as f:
            _yaml.safe_dump(parsed, f)
        # Classifier loads + compiles rules in __init__, so the tmpdir
        # can be cleaned up safely after this call returns.
        return Classifier(rules_path=tmp)


@app.route("/api/classify-test", methods=["POST"])
def classify_test():
    """Mock sandbox classifier.

    Uses the real RACE Classifier loaded from ../config/rules so the
    mock dashboard's rule-test panel reflects production behaviour.
    Falls back to a small keyword heuristic only if the real classifier
    can't be loaded (missing config dir, import error, etc.) — and logs
    a clear warning the first time the fallback fires.
    """
    body = request.get_json() or {}
    ...
    if len(console_text) > 2 * 1024 * 1024:
        return jsonify({"error": "console_text exceeds 2 MB cap"}), 400

    real_classifier = _get_real_classifier()

    # Real classifier path — handles both the no-candidate and the
    # candidate-overlay cases. Candidate path mirrors race/routes/classify_test.py:
    # copy live rules to a tmpdir, add the candidate as a zz-prefixed file,
    # build a transient Classifier, classify once, return.
    if real_classifier is not None:
        if candidate_yaml:
            try:
                sandbox_clf = _build_sandbox_classifier(candidate_yaml)
            except Exception as exc:
                return jsonify({"error": f"Candidate YAML failed to load: {exc}"}), 400
            result = sandbox_clf.classify(console_text)
            return jsonify(_shape_classify_result(result, len(sandbox_clf.rules), True)), 200
        result = real_classifier.classify(console_text)
        return jsonify(_shape_classify_result(result, len(real_classifier.rules), False)), 200

    # Heuristic fallback so the panel still demos when the real
    # classifier can't be loaded (e.g. running mock without venv).
    low = console_text.lower()
    if "nullpointerexception" in low:
        primary = {"label": "Mock NPE Match", "rule_name": "mock_npe",
                   "domain": "Automation / Framework", "subcategory": "Null Pointer",
                   "matched_pattern": "NullPointerException", "impact": "Test Issue",
                   "confidence": "High", "action": "Inspect null source"}
    elif "timeout" in low:
        primary = {"label": "Mock Timeout", "rule_name": "mock_timeout",
                   "domain": "Timeout", "subcategory": "Wait Timeout",
                   "matched_pattern": "timeout", "impact": "Test Issue",
                   "confidence": "Medium", "action": "Investigate slow operation"}
    else:
        return jsonify({
            "matched": False, "candidate_used": bool(candidate_yaml),
            "rule_count": 1, "primary": None, "all_labels": [],
            "evidence_snippet": console_text[:200], "evidence_detail": None,
        }), 200

    return jsonify({
        "matched": True, "candidate_used": bool(candidate_yaml),
        "rule_count": 1, "primary": primary,
        "all_labels": [{
            "label": primary["label"], "domain": primary["domain"],
            "action": primary["action"], "rule_name": primary["rule_name"],
        }],
        "secondary_hint": None,
        "evidence_snippet": console_text[:200],
        "evidence_detail": None,
    }), 200


def _shape_classify_result(result, rule_count, candidate_used):
    """Mirror the real /api/classify-test JSON shape."""
    if not result or not result.matched_rule_name:
        return {
            "matched": False, "candidate_used": candidate_used,
            "rule_count": rule_count, "primary": None, "all_labels": [],
            "evidence_snippet": result.evidence_snippet if result else "",
            "evidence_detail": None,
        }
    conf = result.confidence
    return {
        "matched": True, "candidate_used": candidate_used,
        "rule_count": rule_count,
        "primary": {
            "label": result.label, "rule_name": result.matched_rule_name,
            "domain": result.primary_domain, "subcategory": result.subcategory,
            "matched_pattern": result.matched_pattern, "impact": result.impact,
...
# Effective auto-refresh interval (mirrors the backend behaviour) —
# clamped 5 s – 10 min, default 30 s, env override RACE_AUTO_REFRESH_INTERVAL_MS.
def _mock_auto_refresh_interval_ms():
    raw = os.environ.get("RACE_AUTO_REFRESH_INTERVAL_MS", "").strip()
    if raw:
        try:
            n = int(raw)
            if 5000 <= n <= 600000:
...
def get_config():
    """Return mock config - now includes auto_refresh_interval_ms (#276)."""
    return jsonify({
        "thread_pool_size": 15,
        "default_timeout": 30,
        "auto_refresh_interval_ms": _mock_auto_refresh_interval_ms(),
