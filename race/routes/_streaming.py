"""SSE generator shared by the full-fetch and refresh endpoints. Yields
discovery, per-job, progress, and completion events, and cancels in-flight
work when a new operation supersedes the current one.
"""
...
from race.jenkins_client import JenkinsClient  # noqa: F401  (re-exported for type hints)
...
def _run_producer_with_sentinel(target, args, event_queue: "queue.Queue") -> threading.Thread:
    """Spawn *target* on a daemon thread and always enqueue _STREAM_END after.
    The try/finally guarantees the sentinel reaches the consumer even on failure.
    """
    def _runner():
        try:
            target(*args)
...
def stream_full_fetch(
    ...
    """Full-fetch SSE generator. Clears the store and emits a rich completion summary."""
...
def stream_selective_refresh(
    ...
    """Selective-refresh SSE generator. Updates the store in place with a minimal summary."""
    yield from _stream_pipeline(
        operation_id,
        orchestrator,
        jobs,
...
def create_error_record(event: SSEEvent) -> JobRecord:
    """Build an error JobRecord from a JOB_ERROR event for the in-memory store."""
    return JobRecord(
...
def find_record_from_event(
    ...
    """Retrieve the full JobRecord from the orchestrator, preferring its stored
    record so typed fields Stage 2 needs aren't lost when rebuilding from payload.
    """
    try:
        job_url = event.job_id
...
def _stream_pipeline(
    ...
    def on_result(event: SSEEvent) -> None:
        """Enqueue events from orchestrator worker threads onto the shared queue."""
